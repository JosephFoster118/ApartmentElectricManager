...
Robot::Robot(): gsi::PeriodicThread("MainRobotThread",0.01)
{
	...
	comms = new gri::Communication(this);
	comms->start();
...
{
	RobotPacket* pack = static_cast<RobotPacket*> (param);
	printf("Robot::%s::%d: Network Callback called\n",__INFO__);
	printf("Robot::%s::%d: Packet size [%d]\n",__INFO__,pack->getPacketSize());
	printf("Robot::%s::%d: Packet type [%d]\n",__INFO__,pack->getPacketType());
	char data[1024];
	memset(data,0,1024);
	memcpy(data,pack->getData(),pack->getPacketSize());
	
	switch(pack->getPacketType())
	{
		case RobotPacket::HEART_BEAT:
		{
			printf("Robot::%s::%d: Heart beat recieved\n",__INFO__);
		}break;
		case RobotPacket::MODE_CHANGE:
		{
			...
			printf("Robot::%s::%d: Mode shanged recieved\n",__INFO__);
